...
from dzswj_autotest_unittest.common.dl_session import Login_get_session
...
# 继承获取登录会话
class Api_Auth_Service(Login_get_session):

    def api_getBsjdResult(self, body):
        # Api_Auth_Service().test_login()
        ...
        response = self.session.request(method='post', url=url, json=body, verify=False)
        response = response.text
        return response

    def api_getBsjdResult1(self, body):
        # Api_Auth_Service().test_login()
        # 读取conftest.py文件地址进行拼接
        body = body
        # print(body)
